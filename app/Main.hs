...
init = do
  ...
  putStr "B DECK IP: "
  deckB <- getLine
  return $ NE.fromList [deckA, deckB]
...
main = do
  ...
  mapM_ (`hSetBuffering` NoBuffering) [logMainHandle, logNetworkHandle]
  decks <- Main.init
...
drawFileLoader file deckActives = do
  ...
  liftIO $ update vty (picForImage $ vertJoin line1 line2)
...
interpretInputAsHash ih key = do
  case key of 
    (KChar ch) -> do
      let msg = ih ++ pure ch
          adlerDigest = adler32 $ BSC8.pack msg
